...
const TodosInfinite = () => {
  ...
  const { mutate: workingTodos_Mutate, isLoading: workingLoading } =
    useMutation(getTodosWorking, {
      onSuccess: (data) => {
        queryClient.invalidateQueries("todos");
        dispatch(loadTodosWorking(data));
      },
    });
  const { mutate: doneTodos_Mutate, isLoading: doneLoading } = useMutation(
    getTodosDone,
    {
      onSuccess: (data) => {
        queryClient.invalidateQueries("todos");
  ...
  const onChangeViewMethod = useCallback(
    (num: number) => {
      ...
      {viewMode === 2 && viewMethod === 1 && (
        <>
          ...
          <div
            id="bottom"
            style={{ height: "200px" }}
            ref={hasMoreTodos && !workingLoading ? ref : undefined}
          />
        </>
      )}
      {viewMode === 3 && viewMethod === 1 && (
        <>
          ...
          <div
            id="bottom"
            style={{ height: "200px" }}
            ref={hasMoreTodos && !doneLoading ? ref : undefined}
          />
        </>
      )}
    </>
